...
# If index.html doesn't exist (e.g. during compilation), we define a fallback or use the content directly
...
<head>
    ...
    <style>
        ...
        input[type="text"] {
            flex: 1; background: rgba(0,0,0,0.2); border: 1px solid var(--glass-border); padding: 0.8rem; border-radius: 10px; color: white; outline: none;
        }

        ul { list-style: none; padding: 0; }
        li {
            background: var(--glass-bg); padding: 1rem; margin-bottom: 0.5rem; border-radius: 10px; display: flex; justify-content: space-between; align-items: center; border: 1px solid var(--glass-border);
        }

        .delete-btn {
            background: rgba(255, 0, 0, 0.3); border: none; color: white; padding: 5px 10px; border-radius: 5px; cursor: pointer;
        }
    </style>
</head>
<body>
    ...
    <script>
        ...
        function toggleTimer() {
            if (timerId) {
                ...
                timerId = setInterval(() => {
                    ...
                    else { clearInterval(timerId); alert('Temps écoulé !'); }
                }, 1000);
                document.getElementById('timer-toggle').textContent = 'Pause';
            }
        }
        function resetTimer() {
            clearInterval(timerId); timerId = null; timeLeft = 25 * 60; updateTimerDisplay();
        ...
        function addHabit() {
            ...
            if (input.value) {
                habits.push(input.value); localStorage.setItem('aion_habits', JSON.stringify(habits));
                input.value = ''; renderHabits();
            }
        }
        function deleteHabit(index) {
            habits.splice(index, 1); localStorage.setItem('aion_habits', JSON.stringify(habits));
            renderHabits();
        }
        renderHabits();
    </script>
...
if __name__ == "__main__":
    webview.create_window('AionMind', html=html_content, width=1000, height=800, resizable=True)
    webview.start()
